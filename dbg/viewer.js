window.__log = []; const DBG = (...a) => window.__log.push(Math.round(performance.now()) + ' ' + a.join(' '));
...
if ('serviceWorker' in navigator && location.protocol === 'https:') {
  (() => ({ catch() {} }))(new URL('sw.js', import.meta.url)).catch(() => {});
}
...
function renderPage(n, scale, q = 1) {
  ...
  const promise = (async () => {
    ...
    DBG('render start', n, q, canvas.width, canvas.height);
  ...
  pending.set(k, { promise, cancel() { DBG('cancel', k); stop = true; task?.cancel(); } });
  return promise;
}
...
async function show(s) {
  const token = ++showToken; DBG('show', s, 'token', token);
  start = s;
  const { a, b, scale, L, R, single } = await layout(s);
  DBG('layout', a, b, scale, 'token', token, 'cur', showToken);
  ...
  try {
    ...
    DBG('show catch', e?.name, e?.message, 'token', token, showToken);
